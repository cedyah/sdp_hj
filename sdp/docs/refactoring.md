...
## 12. 주문서 화면 서비스 분리 (Sdpa0020)

컨트롤러 476줄 → 266줄, [`OrderService`](../src/main/java/com/jebi/sdp/service/OrderService.java) 250줄.
`Sdpa0040` 과 같은 방식이다(11절 참고).

- 저장 프로시저 호출 12개를 서비스로 옮겼다. 파라미터(키·값)는 스크립트로 원본과 대조해 확인했다.
- 공통코드 조회는 `Sdpa0040` 과 겹쳐서 [`CodeService`](../src/main/java/com/jebi/sdp/service/CodeService.java) 로 따로 뺐다.
- 작성/수정/삭제에서 헤더 파라미터가 조금씩 다르다. 기존 동작을 그대로 두고 메서드를 나눴다.
  - 작성: 배송지 = `주소1 + " " + 주소2`, 우편번호·주소 항목은 넘기지 않음
  - 수정: 배송지 = `(우편번호)주소1 주소2`
  - 삭제: 배송지 = 화면의 배달장소 값

> **달라지는 동작 두 가지**
>
> 1. 작성 시 `procedure_insertOrderSub` 호출을 감싸던 `try/catch` 를 없앴다.
>    기존에는 예외가 나도 무시하고 다음 품목으로 넘어갔다(로그 한 줄만 남았다).
>    지금은 바깥 `catch` 로 넘어가 오류 화면으로 간다.
> 2. `OUT_PARAM` 이 `null` 일 때 NPE 대신 오류 화면으로 간다(11절과 같다).
